...
function createPdf(data, logoPath = null) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const safeName = (data.nom || 'reponse').replace(/[^a-z0-9]/gi, '_').toLowerCase();
            const filePath = path.join(tmpDir, `questionnaire_${safeName}_${Date.now()}.pdf`);
            ...
            try {
                ...
                console.log('⚠️ Police non trouvée, utilisation Helvetica');
                doc.font('Helvetica');
            }
            ...
            // ========== TABLEAU ==========
            // Utiliser la position Y après le header
            const table = {
                startX: 100,
                y: doc.y + 20, // Position après le header
                colQuestionWidth: 200,
                colAnswerWidth: 250,
                rowHeightMin: 30,
                cellPadding: 6,
                pageBottomLimit: 700,
            ...
            console.log ("Je suis dans createPdf table = ", table);
            // Dessiner l'en-tête du tableau
            drawTableHeader(doc, table);

            // ✅ UTILISER data.questionnaire DIRECTEMENT (c'est déjà un tableau)
            ...
            if (questions.length > 0) {
                ...
                doc.font('Roboto')
                   .fontSize(12)
                   .text('Aucune réponse disponible', table.startX, table.y);
            }
            ...
            console.error('❌ Erreur createPdf:', error);
            reject(error);
        }
    });
}
